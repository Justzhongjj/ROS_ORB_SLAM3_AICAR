...
Sophus::SE3f System::TrackRGBD(const cv::Mat &mask,const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=RGBD  && mSensor!=IMU_RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(mask,imToFeed,imDepthToFeed,timestamp,filename);
