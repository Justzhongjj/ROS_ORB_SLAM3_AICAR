 ...
 */
 ...
 Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &mask, const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    if (mSensor == System::RGBD){
        mCurrentFrame = Frame(mask, mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    }
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mask,mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // Step 6.2 运动模型是空的并且imu未初始化或刚完成重定位，跟踪参考关键帧；否则速模型跟踪
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(mLastFrame.isSet() && mCurrentFrame.isSet())
            {
                ...
                mInvVelocity=mVelocity.inverse();
                //modify
                //数据类型转换并求逆矩阵
                // 创建一个4x4的cv::Mat，类型为float，初始化为0  
                mT = cv::Mat::zeros(4, 4, CV_32F);  
            
                // 填充旋转部分（左上角3x3矩阵）  
                for (int i = 0; i < 3; ++i) {  
                    for (int j = 0; j < 3; ++j) {  
                        mT.at<float>(i, j) = mVelocity.matrix3x4()(i, j);  
                    }  
                }  
                // 填充平移部分（右侧3x1列向量）  
                mT.at<float>(0, 3) = mVelocity.translation()(0);  
                mT.at<float>(1, 3) = mVelocity.translation()(1);  
                mT.at<float>(2, 3) = mVelocity.translation()(2);  
            
                // 最后一行通常为[0 0 0 1]，但在这里由于我们初始化为0，并且只修改了前3行，所以最后一行已经是[0 0 0 0]  
                // 但对于齐次坐标，我们需要将最后一个元素设置为1  
                mT.at<float>(3, 3) = 1.0f;
                cv::invert(mT , minvT, cv::DECOMP_LU);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if(!mCurrentFrame.mMask.empty()){
        //modify
        //在增加背景的特征点
        reprojectStaticPoints();
    //
    }
...
1.将当前帧的掩码投影到上一帧
2.求当前帧与上一帧的掩码的交集
3.将当前帧的掩码减去交集区域作为搜索的目标区域
4.将目标区域搜索得到的特征点及其描述子反投影到到当前帧
5.输出数量
*/
...
// 重投影掩码  
cv::Mat Tracking::reprojectMask() {  
    
    // 初始化上一帧的掩码矩阵，与prevIntrinsics.size相同，类型与currentMask相同  
    cv::Mat prevMask = cv::Mat::zeros(mLastFrame.mMask.size(), mLastFrame.mMask.type());  
    // 遍历当前帧的掩码和深度图 

    for (int y = 0; y < mCurrentFrame.mMask.rows; ++y) {  
        for (int x = 0; x < mCurrentFrame.mMask.cols; ++x) {  
            if (mCurrentFrame.mMask.at<uchar>(y, x) != 0) { // 当前掩码中的目标区域  
                double depth = mCurrentFrame.mDepth.at<float>(y, x); // 假设深度图是float类型  
                if (depth > 0&& depth <100) { // 确保深度有效  
                    // 将2D像素坐标转换为3D世界坐标  
                    Sophus::Vector3f point3D_camera(
                        (x - mCurrentFrame.cx) * depth / mCurrentFrame.fx,
                        (y - mCurrentFrame.cy) * depth / mCurrentFrame.fy,
                        depth
                    ) ;  
                    Sophus::Vector4f point3D_homogeneous(point3D_camera[0], point3D_camera[1], point3D_camera[2], 1.0);
                    // 转换到上一帧的坐标系  
                    Sophus::Vector4f point3D_prev_homogeneous = mVelocity * point3D_homogeneous;
                    Sophus::Vector3f point3D_prev(point3D_prev_homogeneous[0] / point3D_prev_homogeneous[3],  
                              point3D_prev_homogeneous[1] / point3D_prev_homogeneous[3],  
                              point3D_prev_homogeneous[2] / point3D_prev_homogeneous[3]);
                    // 重新投影到上一帧的2D像素坐标  
                    cv::Point2f pixel_prev ;  
                    pixel_prev.x = mLastFrame.fx * point3D_prev[0] / point3D_prev[2] + mLastFrame.cx;  
                    pixel_prev.y = mLastFrame.fy * point3D_prev[1] / point3D_prev[2] + mLastFrame.cy;
  
                    // 检查新坐标是否在有效范围内并四舍五入  
                    int roundedX = static_cast<int>(round(pixel_prev.x));  
                    int roundedY = static_cast<int>(round(pixel_prev.y));  
                    // 如果在有效范围内，则设置掩码值  
                    if (roundedX >= 0 && roundedX < prevMask.cols &&  
                        roundedY >= 0 && roundedY < prevMask.rows) {  
                        prevMask.at<uchar>(roundedY, roundedX) = 255;  
                    }  
                }  
            }  
        }  
...
void Tracking::reprojectStaticPoints(){

    // 创建一个与目标大小相同的空白掩码  
    cv::Mat projectMask = reprojectMask();  
    if(projectMask.empty()){
         std::cout<<"projectMask is empty"<<std::endl;
        return;
    }
    ...
    cv::Mat intersection;  
    // 使用bitwise_and求重投影掩码与上一帧掩码的交集  
    cv::bitwise_and(projectMask, mLastFrame.mMask, intersection);
    ...
    cv::Mat dstMask=projectMask-intersection;
    mCurrentFrame.mvKeysUn.reserve(mCurrentFrame.mvKeysUn.size()+300);
    vector<cv::KeyPoint>::iterator it=mLastFrame.mvKeysUn.begin(); //查找target
    for (const auto& kp : mLastFrame.mvKeysUn) { 
       
        // 注意：KeyPoint的pt成员变量是cv::Point2f类型，包含浮点坐标  
        // 需要将其转换为整数坐标以访问掩码图像  
        int x = static_cast<int>(kp.pt.x + 0.5f); // 加0.5并转换为int以四舍五入  
        int y = static_cast<int>(kp.pt.y + 0.5f);  
        double depth = mLastFrame.mDepth.at<float>(y, x); // 假设深度图是float类型  
        if (depth < 0 || depth >100)
            continue;// 确保深度有效
        // 检查坐标是否在掩码图像范围内  
        // 检查该位置的像素值是否等于maskValue
        if (x >= 0 && x < dstMask.cols && y >= 0 && y < dstMask.rows && dstMask.at<uchar>(y, x) !=0) {  
            //找特征点对应的描述子
            
	        int index = distance(mLastFrame.mvKeysUn.begin(), it); 
            
            // 将2D像素坐标转换为3D世界坐标  
            Sophus::Vector3f point3D_camera(
                (x - mLastFrame.cx) * depth / mLastFrame.fx,
                (y - mLastFrame.cy) * depth / mLastFrame.fy,
                depth
            ) ;
            Sophus::Vector4f point3D_homogeneous(point3D_camera[0], point3D_camera[1], point3D_camera[2], 1.0);
            // 转换到当前帧的坐标系  
            Sophus::Vector4f point3D_prev_homogeneous = mInvVelocity * point3D_homogeneous;
            Sophus::Vector3f point3D_prev(point3D_prev_homogeneous[0] / point3D_prev_homogeneous[3],  
                      point3D_prev_homogeneous[1] / point3D_prev_homogeneous[3],  
                      point3D_prev_homogeneous[2] / point3D_prev_homogeneous[3]);
            ...
            // 重新投影到当前帧的2D像素坐标  
            cv::Point2f pixel_prev ;  
            pixel_prev.x = mCurrentFrame.fx * point3D_prev[0] / point3D_prev[2] + mCurrentFrame.cx;  
            pixel_prev.y = mCurrentFrame.fy * point3D_prev[1] / point3D_prev[2] + mCurrentFrame.cy;
            // 检查新坐标是否在有效范围内并四舍五入  
            int roundedX = static_cast<int>(round(pixel_prev.x));  
            int roundedY = static_cast<int>(round(pixel_prev.y));
            
            // 边界检查  
            if (roundedX >= 0 && roundedX < mImGray.cols && roundedY >= 0 && roundedY < mImGray.rows) {  
                cv::KeyPoint projectedKp(static_cast<float>(roundedX), static_cast<float>(roundedY), kp.size, kp.angle,kp.response,kp.octave,kp.class_id);
                //_mvKeys.push_back(projectedKp);
                //_mDescriptors.push_back(mLastFrame.mDescriptors.row(index));
                //mCurrentFrame.mvKeysUn.push_back(projectedKp);
                //mCurrentFrame.mDescriptors.push_back(mLastFrame.mDescriptors.row(index));
            }
            flage=true;
            size++;
        }  
    ...
    // 没有特征点在掩码目标区域内 
    if(flage==false){
        std::cout<<"it don't exist static point"<<std::endl;
    }
    else{
        std::cout<<"size of staticPoints : "<<size<<std::endl;
        //mCurrentFrame.mvKeysUn.reserve(mCurrentFrame.mvKeysUn.size()+_mvKeys.size());
        //mCurrentFrame.mvKeysUn.insert(mCurrentFrame.mvKeysUn.end(),_mvKeys.begin(),_mvKeys.end());
        //cv::vconcat(mCurrentFrame.mDescriptors, _mDescriptors,mCurrentFrame.mDescriptors);
